 ...
 */
void EncryptDES(bool key[56], bool outBlk[64], bool inBlk[64], int verbose, int numRounds) {
  int i,round;
 ...
 */
void DecryptDES(bool key[56], bool outBlk[64], bool inBlk[64], int verbose, int numRounds) {
  int i,round;
  bool R[32], L[32], fout[32];
  bool roundKey[56];
...
//encrypting message
char* encrypt_msg(char* message, char* enc_msg,  char* key, long msg_size, int numRounds){
   //ARGUMENTS	
  //==========================
  //printf("Allocating memory\n"); 
  //char *enc_msg = calloc(1, msg_size+1); //will be returned with encrypted message
  char * message_part = calloc(64, 1); //will hold the parts of the message
  int i,j; //iterator key

  //LOGIC PART
  //============================

  printf("Num of iteration: %ld", (msg_size/8));
  ...
  for(j=1;j<=(msg_size/8);j++){
	...
	//transform char to bool to be processed by the functions 
	bool * message_in = (bool*)message_part;
	bool * message_out = calloc(64, 1); //alloc 8 bytes = 64 bits
	bool * key_bool = calloc(56, 1); 
	memcpy(key_bool, key, strlen(key));
	
	
//EncryptDES(bool key[56], bool outBlk[64], bool inBlk[64], int verbose, int numRounds)

	EncryptDES(key_bool, message_out, message_in, 1, numRounds);

	char * message_char = (char*)message_out;
	//strcat(enc_msg, message_char);
	memcpy(&enc_msg[(j-1)*8], message_char, 8);

	free(key_bool); free(message_out);
  }
  free(message_part);
  return enc_msg;
}
...
int main(int argc, char ** argv){
	...
	/*
	unsigned char * final_result = calloc(1, lSize+1);
	XTeaDecryptECB(enc_msg, lSize,
                       final_result, lSize,
                       key);

...
int main( int argc, char *argv[] )
{
	...
	unsigned char * enc_msg; //message tha will receive the encryption of each part of the message.
	unsigned char msg_part[16]; //variable that will receive the blocks of message
	...
	while(i<=lSize){
		...
		//xor between msg_part and md5_sum
		for(j=0; j<16; j++)
				msg_part[j] = (char)(msg_part[j]^md5_sum[j]);


		//update ctx with the message
		MD5_Update( &ctx, msg_part, 16, rounds);

		//finish the processing and save the has into md5_sum
		MD5_Final( md5_sum, &ctx, rounds);

		//copy the hash code to enc_msg[i]
